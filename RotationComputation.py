...
def FromEuler_Angle_in_Rad2Rotation (Euler_vec):
    r = Rotation.from_euler('zyx', Euler_vec, degrees=False)
    Rotation_mat = np.mat( r.as_matrix() )
    return Rotation_mat

def CalcAngularVelocity_in_Rad_From_RotationMatrixDiff( Last_Rotation_Mat, Current_Rotation_Mat, Calc_gap):
    Rot_diff = Current_Rotation_Mat * Last_Rotation_Mat.T
    Rotation_diff_for_Quat = Rotation.from_matrix(Rot_diff)
    Quaternion_diff = Rotation_diff_for_Quat.as_quat()
    axis = 1 / (np.linalg.norm(Quaternion_diff[0:3]) +
                EXTREME_SMALL_NUMBER_4_ROTATION_COMPUTATION) * Quaternion_diff[0:3]
    angle_as_array = 2 * np.arccos(Quaternion_diff[3]) / Calc_gap * axis
    return np.mat(angle_as_array).T

def GetUnitDirection_Safe(Arrow):
    return Arrow/ ( np.linalg.norm(Arrow) + EXTREME_SMALL_NUMBER_4_ROTATION_COMPUTATION)

def Normalize_Rot_Mat_Safe(Rot_Mat):
    First_column_norm = np.linalg.norm([Rot_Mat[0, 0], Rot_Mat[1, 0], Rot_Mat[2, 0] ])+ \
                        EXTREME_SMALL_NUMBER_4_ROTATION_COMPUTATION
    Second_column_norm = np.linalg.norm([Rot_Mat[0, 1], Rot_Mat[1, 1], Rot_Mat[2, 1]])+ \
                         EXTREME_SMALL_NUMBER_4_ROTATION_COMPUTATION
    Third_column_norm = np.linalg.norm([Rot_Mat[0, 2], Rot_Mat[1, 2], Rot_Mat[2, 2]])+ \
                        EXTREME_SMALL_NUMBER_4_ROTATION_COMPUTATION
    Norm_Rot_Mat = np.mat( [ [Rot_Mat[0, 0] / First_column_norm,
                              Rot_Mat[0, 1] / Second_column_norm,
                              Rot_Mat[0, 2] / Third_column_norm],
                             [Rot_Mat[1, 0] / First_column_norm,
                              Rot_Mat[1, 1] / Second_column_norm,
                              Rot_Mat[1, 2] / Third_column_norm],
                             [Rot_Mat[2, 0] / First_column_norm,
                              Rot_Mat[2, 1] / Second_column_norm,
                              Rot_Mat[2, 2] / Third_column_norm],
                             ])
    return Norm_Rot_Mat
